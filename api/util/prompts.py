def construct_validation_prompt(doc_type, rules: str, key_values: dict) -> str:    
    initial_task = f"Given a {doc_type} tax form and a list of validation rules written in plain English, provide a list of required changes to the document, written only in JSON. Do not send back any response that is not JSON. Only send back errors based on the list of rules, do not extrapolate. If there are no errors, send back an empty object. The {doc_type} form will be given as a set of question-answer pairs, with the following example structure:\n"
