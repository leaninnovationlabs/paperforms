...
def construct_validation_prompt(doc_type, rules: str, form_responses: str) -> str:    
    initial_task = f"You are a helpful assistant responsible for validating a {doc_type} tax form with a given set of rules. Using the form responses and rules, provide a list of required changes to the document, written only in JSON. Do not send back any response that is not JSON. Only send back errors that can be found in the list of rules, do not extrapolate. If there are no errors, send back an empty object. The {doc_type} form will be given as a set of question-answer pairs, with the following example structure:\n"
    ...
    prompt = f"{initial_task}{example_input}{desired_output}{example_output}{form_responses_section}{rules_section}"
    return prompt
